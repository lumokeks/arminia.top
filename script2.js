...
function __h(e) {
    ...
    if(nums.find(e2 => `${e2}`===e)||e==="0") {
        if(e!=="0") {t[i] = Number(e);};
        i++;if(i>80) {let r = new _c12({title: "(!)", data: "Solving Sudoku...", type: "warn"});setTimeout(() => {t = new Sudoku(t).solve();r.close();new _c12({title: "(/)", data: "Successfully solved Sudoku!", duration: 4000, type: "success"});i = -1;}, 1000);};
    } else if(e==="Backspace") {
        t[i] = undefined;
        i--;if(i<0) i = 0;
    };
